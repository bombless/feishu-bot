...
function normalizeCwd (input) {
  ...
  if (lower !== rootLower && !lower.startsWith(rootLower + path.sep)) {
    throw new Error('cwd 必须位于 Windows D: 盘内')
  }
...
function listDirectory (input) {
  ...
  return {
    cwd,
    entries: fs.readdirSync(cwd, { withFileTypes: true }).map(entry => ({
      name: entry.name,
...
function createAgentServer ({ store, port = Number(process.env.AGENT_PORT || 8787), host = process.env.AGENT_HOST || '127.0.0.1' }) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/agent/list') {
        return send(res, 200, listDirectory(url.searchParams.get('path') || 'D:\\'))
      }
      if (req.method === 'GET' && url.pathname === '/agent/history') {
        return send(res, 200, { tasks: store.listTasks(url.searchParams.get('limit')) })
      }
...
module.exports = { createAgentServer, listDirectory, normalizeCwd }
